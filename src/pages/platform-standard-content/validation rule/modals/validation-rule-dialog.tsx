    ...
    if (isEdit && validationRule) {
      updateRule(
        ...
        },
        {
          ...
          onError: (err: unknown) => {
            let errMsg = "Failed to update validation rule."
      ...
      createRule(payload, {
        ...
        onError: (err: unknown) => {
          let errMsg = "Failed to create validation rule."
          const axiosErr = err as { response?: { data?: { detail?: unknown; message?: unknown } } }
          if (axiosErr.response?.data?.detail) {
            errMsg = String(axiosErr.response.data.detail)
          } else if (axiosErr.response?.data?.message) {
    ...
    >
      <DialogContent className="max-h-[85vh] gap-0 overflow-hidden p-0 sm:max-w-[800px] w-11/12">
        <DialogHeader className="border-b border-border px-6 py-5">
          ...
          <DialogDescription>
            {isEdit
              ? "Update the configuration, engine settings, and supported targets for this validation rule."
              : "Add a new declarative validation rule to validate document extracted fields."}
          </DialogDescription>
        </DialogHeader>
