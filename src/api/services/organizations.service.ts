...
export const organizationsService = {
  ...
  replicateMasterData: async (
    tenantId: string,
    payload?: Record<string, boolean>
  ): Promise<unknown> => {
    const response = await api.post<unknown>(
      `/api/${tenantId}/replicate`,
      payload ?? ""
    )
    return response.data
  },
  ...
  updateTenantConfigurations: async (
    tenantId: string,
    payload: { values: Record<string, string> }
  ): Promise<Configuration[]> => {
    const response = await api.post<Configuration[]>(
  ...
  assignPlan: async (
    tenantId: string,
    payload: { plan_id: string; valid_from?: string; valid_to?: string }
  ): Promise<any> => {
    const response = await api.post<any>(
